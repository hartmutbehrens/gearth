...
my $dbh = connect_db();
die "Could not connect alcatelRSA_3G database! Please check settings in connect_db() subroutine!\n" unless defined $dbh;


my $OutFile = '/var/www/html/VodacomSites3G.kml';
if (MAKE_MYPLACES) {
	$OutFile = '/var/tools/radar/etc/myplaces.kml';
}
my $count = 0;
my $IconDir = "./icons";
my $tname = "CELLPOSITIONS_3G";
...
TILLMPL1
 ...
 <Document>
  <name>Vodacom 3G Sites $tdstr</name>
...
TILLENDICON
	...
	my %sites = ();
		...
		if (array_exists(\@tables,$tname)) {
			...
			$sth = $dbh->prepare("select NODEB_NAME, LAC, CI, LAT, LON, ASIMUTH, DOWNTILT, HEIGHT FROM $tname WHERE LAT <> '' AND LON <> '' AND CI <> '' AND RNC like '%$region%';");
			if ($sth) {
				$sth->execute;
				while ( my @row = $sth->fetchrow_array ) {
					$sites{$row[0]}{$row[1].'/'.$row[2]} = [ (@row[0..7]) ];
					#print "@row\n";
				}
			}
			else {
...
TILLR
		...
		SITE: for my $site (sort {$sites{$a}->{(keys %{$sites{$a}})[0]}[2] cmp $sites{$b}->{(keys %{$sites{$b}})[0]}[2]} keys %sites) {
			my $size = 48;
			my $sname = "";
			my %cis = ();
			my ($lat,$lon);
			...
			for my $sector (sort keys %{$sites{$site}}) {
				#next SITE if ($sites{$site}->{$sector}[0] ne $region);
				last if (!$maxsectors--);
				push @azimuths, $sites{$site}->{$sector}[5];
				$sname = $sites{$site}->{$sector}[0] . ' - ' . $sites{$site}->{$sector}[2];
				$cis{$sector} = $sites{$site}->{$sector}[5];
...
TILLHERED1
			for my $ci (sort keys %cis) {
				print OUTFILE "CI:$ci @ $cis{$ci}, SC: ".$sites{$site}->{$ci}[6].", RNC: ".$sites{$site}->{$ci}[7]."<br /><br />\n";
			}
...
## connect to db
sub connect_db {
  my $dsn = 'DBI:mysql:alcatelRSA_3G;host=127.0.0.1;port=3306';
